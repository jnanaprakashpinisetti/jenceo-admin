...
// Import components from Invoice folder
import CustomInvoiceForm from './Invocie/CustomInvoiceForm';
import ThankYouMessageForm from './Invocie/ThankYouMessageForm';
import InvoiceHistoryTable from './Invocie/InvoiceHistoryTable';
import DeletedInvoicesTable from './Invocie/DeletedInvoicesTable';
import PaymentModal from './Invocie/PaymentModal';
import { DeleteConfirmModal, RestoreConfirmModal } from './Invocie/ConfirmModals';
...
import {
  ...
  getCurrentThankYouMessage,
  getPaymentDetails
} from './Invocie/invoiceUtils';
...
const CompanyInvoice = ({
  ...
  const enhancedResolvePhoto = (photo) => {
    return resolvePhoto(photo, defaultCompanyPhoto);
  };
      ...
      for (const department of departments) {
        ...
        if (data) {
          ...
          if (workerKey) {
            ...
            if (photoData) {
              workerData.photo = enhancedResolvePhoto(photoData);
            }
          }
          ...
          if (!workerData.photo && company?.companyId) {
            ...
            if (companyInfo) {
              ...
              if (companyWorker) {
                workerData.photo = enhancedResolvePhoto(
    ...
    try {
      ...
      const invoiceToSave = {
        ...
        data: {
          ...
          workerPhoto: enhancedResolvePhoto(invoiceObj.data.workerPhoto || ''),
          ...
          workerSnapshot: invoiceObj.data.workerSnapshot || {
            ...
            photo: enhancedResolvePhoto(invoiceObj.data.workerPhoto || '')
          }
        }
      };
  ...
  // Determine service type for thank you message
  const determineCompanyServiceType = () => {
    return determineServiceType(company?.companyType);
  };

  // Get current thank you message
  const getCurrentThankYou = () => {
    return getCurrentThankYouMessage(
      invoiceData.thankYouType,
      invoiceData.customThankYou,
      company?.companyName
    );
  };
  ...
  useEffect(() => {
    if (company?.companyId) {
      ...
      setInvoiceData(prev => ({
        ...
        thankYouType: determineCompanyServiceType(),
        invoiceNumber: ''
      }));
    }
  }, [company]);
      ...
      const updatedInvoice = {
        ...
        data: {
          ...
          workerSnapshot: {
            ...
            photo: enhancedResolvePhoto(invoiceData.workerPhoto)
          }
        },
  ...
  const handleApplyCustomInvoice = (formData) => {
    setInvoiceData({
      ...
      workerPhoto: enhancedResolvePhoto(formData.workerPhoto || invoiceData.workerPhoto),
      ...
      workerSnapshot: {
        ...
        photo: enhancedResolvePhoto(formData.workerPhoto || invoiceData.workerPhoto)
      }
    });
  ...
  const handleThankYouTypeChange = (type, customMessage = '') => {
    setSelectedThankYouType(type);
    setInvoiceData(prev => ({
      ...prev,
      thankYouType: type,
      customThankYou: type === 'custom' ? customMessage : ''
    }));
  ...
  const paymentDetails = useMemo(() => getPaymentDetails(payments, company), [payments, company]);
  ...
  const buildCompanyDetailsTable = () => {
    ...
    const workerPhoto = enhancedResolvePhoto(workerSnapshot.photo || invoiceData.workerPhoto || '');
  ...
  const buildInvoiceHTML = () => {
    ...
    const thankYouMessage = getCurrentThankYou();
...
<div class="page">
 ...
 <div class="invoice-container">
  ...
  ${invoiceHistory.map((invoice) => {
      ...
      const workerPhoto = enhancedResolvePhoto(workerSnapshot.photo || invoice.data.workerPhoto);
...
@media (max-width: 768px) {
  ...
  return (
    <>
      ...
      {showRestoreConfirm && (
        <RestoreConfirmModal
          invoiceToRestore={invoiceToRestore}
          setShowRestoreConfirm={setShowRestoreConfirm}
          confirmRestoreInvoice={confirmRestoreInvoice}
          formatDate={formatDate}
          formatAmount={formatAmount}
        />
      )}
      ...
      {showCustomInvoiceForm && (
        <CustomInvoiceForm
          ...
          searchWorkerGlobally={searchWorkerGlobally}
          normalizeWorker={normalizeWorker}
          resolvePhoto={enhancedResolvePhoto}
      ...
      {showThankYouMessageForm && (
        <ThankYouMessageForm
          onClose={(type, message) => {
            handleThankYouTypeChange(type, message);
            setShowThankYouMessageForm(false);
          }}
          company={company}
          invoiceData={invoiceData}
          thankYouMessages={thankYouMessages}
          determineServiceType={determineCompanyServiceType}
        />
      )}
          ...
          {activeTab === 'preview' ? (
            ...
            <>
              ...
              <InvoiceHistoryTable
                ...
                handleRestoreInvoice={handleRestoreInvoice}
                ...
                calculateInvoiceAmount={calculateInvoiceAmount}
                resolvePhoto={enhancedResolvePhoto}
                downloadHistoryHTML={downloadHistoryHTML}
              />
            </>
          ) : (
            <>
