// src/components/DashBoard/CompanyPaymentCard.jsx
import React, { useEffect, useMemo, useState, useRef } from "react";
import PropTypes from "prop-types";
import { useNavigate } from "react-router-dom";
import { COMPANY_PATHS } from "../../utils/dataPaths"; // Assuming you have company paths
...
/* ---------------------- SVG Charts ---------------------- */
function BarChart({ data = [], width = 520, height = 150, pad = 30, color = "url(#gradCompany)" }) {
...
/* Normalize company record into payment rows */
function extractPaymentsFromCompany(companyRecord = {}, collectionName = "", department = "") {
  const companyId = companyRecord.idNo ?? companyRecord.id ?? companyRecord.companyId ?? companyRecord.key ?? "";
  const companyName = companyRecord.companyName ?? companyRecord.cName ?? companyRecord.name ?? companyRecord.company_name ?? "Unknown";

  const paymentsArr = Array.isArray(companyRecord.payments)
    ? companyRecord.payments
    : (companyRecord.payments && typeof companyRecord.payments === "object" ? Object.values(companyRecord.payments) : []);
  ...
  if (paymentsArr && paymentsArr.length) {
    paymentsArr.forEach((p) => {
      ...
      const jenceoInvoiceNo = p.jenceoInvoiceNo ?? "";
      const companyInvoiceNo = p.companyInvoiceNo ?? "";
      const employeeId = p.employeeId ?? "";
      const employeeName = p.employeeName ?? "";
      ...
      results.push({
        ...
        remarks,
        raw: p,
        _fullCompanyData: companyRecord
      });
    });
  } else {
    // fallback single record fields
    const singleAmount = safeNumber(companyRecord.paidAmount ?? companyRecord.amount ?? companyRecord.payment ?? 0);
    if (singleAmount !== 0 || singleAmount === 0) {
      const date = companyRecord.date ?? companyRecord.paymentDate ?? "";
      results.push({
        sourceCollection: collectionName,
        department: department || "",
        companyId: String(companyId || ""),
        ...
        balance: safeNumber(companyRecord.balance ?? 0),
        refundAmount: safeNumber(companyRecord.refundAmount ?? 0),
        isRefund: Boolean(companyRecord.refund ?? companyRecord.refundAmount),
        receipt: companyRecord.receptNo ?? companyRecord.receiptNo ?? "",
        method: companyRecord.paymentMethod ?? "",
        date,
        parsedDate: parseDateRobust(date),
        jenceoInvoiceNo: companyRecord.jenceoInvoiceNo ?? "",
        companyInvoiceNo: companyRecord.companyInvoiceNo ?? "",
        employeeId: companyRecord.employeeId ?? "",
        employeeName: companyRecord.employeeName ?? "",
        remarks: companyRecord.remarks ?? "",
        raw: companyRecord,
        _fullCompanyData: companyRecord
      });
    }
  ...
  const logsNode = companyRecord.paymentLogs ?? companyRecord.paymentLog ?? companyRecord.payment_logs ?? companyRecord.paymentLogsList ?? null;
  ...
  if (logsArr && logsArr.length) {
    logsArr.forEach((lg) => {
      ...
      const dateLg = lg.date ?? lg.logDate ?? lg.createdAt ?? companyRecord.date ?? null;
      ...
      results.push({
        ...
        companyName: String(companyName || ""),
        paidAmount: 0,
        balance: safeNumber(lg.balance ?? companyRecord.balance ?? 0),
        refundAmount,
        isRefund: true,
        ...
        date: dateLg,
        parsedDate: parsedLg,
        jenceoInvoiceNo: lg.jenceoInvoiceNo ?? "",
        companyInvoiceNo: lg.companyInvoiceNo ?? "",
        employeeId: lg.employeeId ?? "",
        employeeName: lg.employeeName ?? "",
        remarks: lg.remarks ?? "",
        raw: lg,
        _fullCompanyData: companyRecord
      });
...
function groupPaymentsByYearMonth(payments) {
  ...
  (payments || []).forEach((p) => {
    ...
    years[yKey].months[mKey].totals.paid += Number(p.paidAmount ?? 0);
    ...
    years[yKey].totals.paid += Number(p.paidAmount ?? 0);
...
export default function CompanyPaymentCard({
  companyCollections = { active: "CompanyData", exit: "ExitCompanies" },
  openCompanyModal = null,
  rootPathForCompanyProfile = "/company-profile",
}) {
...
useEffect(() => {
  ...
  (async () => {
    ...
    if (!mounted) return;
    if (!fdb) {
      console.error("Firebase DB not found - CompanyPaymentCard");
      setLoading(false);
      return;
    }

    const snapshots = {};
    // FIX: Use only department paths, not root collections (to avoid duplicates)
    const paths = Object.values(COMPANY_PATHS); // Only use department paths
    
    // REMOVED: const paths = [companyCollections.active, companyCollections.exit];
    // REMOVED: Object.values(COMPANY_PATHS).forEach(path => { if (!paths.includes(path)) { paths.push(path); } });
    ...
    const getDepartmentFromPath = (path) => {
      // Find department from COMPANY_PATHS
      for (const [dept, deptPath] of Object.entries(COMPANY_PATHS)) {
    ...
    const rebuild = () => {
      ...
      paths.forEach((p) => {
        ...
        Object.keys(node).forEach((k) => {
          const company = node[k] || {};
          const payments = extractPaymentsFromCompany(company, p, department);
          payments.forEach((pm) => {
            pm._companyDbKey = `${p}/${k}`;
...
}, [companyCollections.active, companyCollections.exit]); // Keep dependencies but only use COMPANY_PATHS internally
  ...
  // company summaries & overall totals
  const companySummaries = useMemo(() => {
    const map = {};
    filteredPayments.forEach((p) => {
      const id = p.companyId || "Unknown";
      const name = p.companyName || "Unknown";
      if (!map[id]) map[id] = { companyId: id, companyName: name, paid: 0, balance: 0, refunds: 0, count: 0 };
  ...
  const overallTotalsAllDepartments = useMemo(() => {
    const acc = allPayments.reduce(
      ...
      { paid: 0, balanceSumAllRows: 0, refunds: 0, count: 0, pendingCount: 0 }
    );

    // Payment (net) across all rows
    acc.netPaid = (allPayments || []).reduce((s, r) => s + getDisplayPayment(r), 0);

    // Outstanding balance = sum of latest balance per company (using sorted allPayments desc)
    const latestByCompany = {};
    (allPayments || []).forEach((row) => {
      const cid = (row.companyId || "Unknown").toString();
      if (latestByCompany[cid] === undefined) {
        latestByCompany[cid] = Number(row.balance || 0);
      }
    });
    acc.netBalance = Object.values(latestByCompany).reduce((s, n) => s + Number(n || 0), 0);
  ...
  const overallTotalsCurrentDepartment = useMemo(() => {
    ...
    // Outstanding balance = sum of latest balance per company (using sorted filteredPayments desc)
    const latestByCompany = {};
  ...
  useEffect(() => {
    ...
    if (filterCompany) {
      const candidate = keys.slice().reverse().find((k) => (monthsObj[k].rows || []).some((r) => (r.companyId || "").toString() === (filterCompany || "").toString()));
      if (candidate) {
  ...
  const currentMonthRows = useMemo(() => {
    ...
    if (!mObj) return [];
    let rows = mObj.rows || [];
    if (filterCompany) rows = rows.filter((r) => (r.companyId || "").toString() === (filterCompany || "").toString());
    // sort newest first
  ...
  }, [activeYear, activeMonth, filterCompany, grouped]);
  ...
  const currentYearRows = useMemo(() => {
    ...
    if (filterCompany) rows = rows.filter((r) => (r.companyId || "").toString() === (filterCompany || "").toString());
    return rows;
  }, [activeYear, grouped, filterCompany]);
  ...
  const paymentMethodDistribution = useMemo(() => {
    ...
    return Object.entries(methods).map(([method, amount]) => ({
      key: method,
      value: amount,
      color: method === "Cash" ? "url(#gradCompany)" :
  ...
  // Company distribution for pie chart
  const companyDistribution = useMemo(() => {
    const companies = {};
    currentMonthRows.forEach(payment => {
      const company = payment.companyName;
      companies[company] = (companies[company] || 0) + getDisplayPayment(payment);
    });
  ...
  const monthlyTotals = useMemo(() => {
    ...
    // Balance (month): sum of latest balance per company within the month (avoid double subtraction)
    const latestByCompanyM = {};
    ...
    for (const row of rowsM) {
      const cid = (row.companyId || "Unknown").toString();
      if (latestByCompanyM[cid] === undefined) latestByCompanyM[cid] = Number(row.balance || 0);
    }
    const balance = Object.values(latestByCompanyM).reduce((s, n) => s + Number(n || 0), 0);
    const pending = currentMonthRows.reduce((s, r) => s + ((Number(r.paidAmount || 0) === 0 && Number(r.refundAmount || 0) === 0 && !isBalancePayRow(r)) ? 1 : 0), 0);
    return { paid, balance, refunds, pending, count: currentMonthRows.length, net };
  }, [currentMonthRows]);
  ...
  const yearlyTotals = useMemo(() => {
    ...
    // Balance (year): sum of latest balance per company within the year
    const latestByCompanyY = {};
    ...
    for (const row of rowsY) {
      const cid = (row.companyId || "Unknown").toString();
      if (latestByCompanyY[cid] === undefined) latestByCompanyY[cid] = Number(row.balance || 0);
    }
    const balance = Object.values(latestByCompanyY).reduce((s, n) => s + Number(n || 0), 0);
    const pending = currentYearRows.reduce((s, r) => s + ((Number(r.paidAmount || 0) === 0 && Number(r.refundAmount || 0) === 0 && !isBalancePayRow(r)) ? 1 : 0), 0);
    return { paid, balance, refunds, pending, count: currentYearRows.length, net };
  }, [currentYearRows]);
  ...
  const topCompanies = useMemo(() => companySummaries.slice(0, 3), [companySummaries]);
  ...
  const exportCSV = (scope = "month") => {
    const rows = [["#", "Department", "Company ID", "Company Name", "Method", "Date", "Receipt No", "Payment", "Balance", "JenCeo Invoice", "Company Invoice", "Employee ID", "Employee Name"]];
    if (scope === "month") {
    ...
    a.href = url;
    a.download = `company-payments-${scope}-${new Date().toISOString().slice(0, 10)}.csv`;
  ...
  const printScope = (scope = "month") => {
    let html = `<html><head><title>Company Payments ${scope}</title>
<style>
  /* Overall cards – purple family */
  .overall-cards .summary-card, .overall-cards .header-gradient { color: #fff; }
  .overall-cards .grad-paid { background: linear-gradient(135deg, #7c3aed, #5b21b6); }
  .overall-cards .grad-balance { background: linear-gradient(135deg, #8b5cf6, #6d28d9); }
  .overall-cards .grad-refund { background: linear-gradient(135deg, #dc2626, #b91c1c); }
  ...
  .invest-table .text-danger { font-weight: 600; }
    ...
    html += `<h3>Company Payments - ${scope} (${activeDepartment})</h3><table><thead><tr><th>#</th><th>Department</th><th>Company ID</th><th>Company Name</th><th>Method</th><th>Date</th><th>Receipt</th><th>Payment</th><th>Balance</th><th>JenCeo Invoice</th><th>Company Invoice</th><th>Employee ID</th><th>Employee Name</th></tr></thead><tbody>`;
    if (scope === "month") {
      currentMonthRows.forEach((r, i) => (html += `<tr><td>${i + 1}</td><td>${r.department || ""}</td><td>${r.companyId || ""}</td><td>${r.companyName || ""}</td><td>${r.method || ""}</td><td>${r.date || ""}</td><td>${r.receipt || ""}</td><td>${getDisplayPayment(r)}</td><td>${getDisplayBalance(r)}</td><td>${r.jenceoInvoiceNo || ""}</td><td>${r.companyInvoiceNo || ""}</td><td>${r.employeeId || ""}</td><td>${r.employeeName || ""}</td></tr>`));
    } else if (scope === "year") {
      currentYearRows.forEach((r, i) => (html += `<tr><td>${i + 1}</td><td>${r.department || ""}</td><td>${r.companyId || ""}</td><td>${r.companyName || ""}</td><td>${r.method || ""}</td><td>${r.date || ""}</td><td>${r.receipt || ""}</td><td>${getDisplayPayment(r)}</td><td>${getDisplayBalance(r)}</td><td>${r.jenceoInvoiceNo || ""}</td><td>${r.companyInvoiceNo || ""}</td><td>${r.employeeId || ""}</td><td>${r.employeeName || ""}</td></tr>`));
    } else {
      filteredPayments.forEach((r, i) => (html += `<tr><td>${i + 1}</td><td>${r.department || ""}</td><td>${r.companyId || ""}</td><td>${r.companyName || ""}</td><td>${r.method || ""}</td><td>${r.date || ""}</td><td>${r.receipt || ""}</td><td>${getDisplayPayment(r)}</td><td>${getDisplayBalance(r)}</td><td>${r.jenceoInvoiceNo || ""}</td><td>${r.companyInvoiceNo || ""}</td><td>${r.employeeId || ""}</td><td>${r.employeeName || ""}</td></tr>`));
    }
  ...
  const handleCompanyProfileClick = (r) => {
    if (openCompanyModal && typeof openCompanyModal === "function" && r.companyId) {
      openCompanyModal(r.companyId);
      return;
    }
    if (navigate) {
      navigate(`${rootPathForCompanyProfile}/${encodeURIComponent(r.companyId || "")}`);
      return;
    }
    window.location.href = `${rootPathForCompanyProfile}/${encodeURIComponent(r.companyId || "")}`;
  };
  ...
  const getPaymentDetails = (payment) => {
    ...
    details.push({ label: "Company ID", value: payment.companyId || "-" });
    details.push({ label: "Company Name", value: payment.companyName || "-" });
    ...
    details.push({ label: "JenCeo Invoice No", value: payment.jenceoInvoiceNo || "-" });
    details.push({ label: "Company Invoice No", value: payment.companyInvoiceNo || "-" });
    details.push({ label: "Employee ID", value: payment.employeeId || "-" });
    details.push({ label: "Employee Name", value: payment.employeeName || "-" });
    if (payment.remarks) details.push({ label: "Remarks", value: payment.remarks });

    // Additional company details from full company data
    const companyData = payment._fullCompanyData || {};
    if (companyData.phone) details.push({ label: "Company Phone", value: companyData.phone });
    if (companyData.email) details.push({ label: "Company Email", value: companyData.email });
    if (companyData.address) details.push({ label: "Address", value: companyData.address });
    if (companyData.contactPerson) details.push({ label: "Contact Person", value: companyData.contactPerson });
  ...
  return (
    <>
      <div className="company-payment-card" onClick={() => setModalOpen(true)} role="button">
        <div className="invest-card__box bg-primary bg-gradient" role="button">
          <div className="invest-card__head">
            <div>🏢</div>
            <div className="invest-card__meta">
              <div className="invest-card__label">Company Payments</div>
              <div className="invest-card__total">{loading ? "Loading..." : formatINR(overallTotalsAllDepartments.netPaid ?? overallTotalsAllDepartments.paid)}</div>
            </div>
          </div>
          ...
          <div className="invest-modal-dialog" ref={modalRef} onClick={(e) => e.stopPropagation()}>
            <div className="invest-modal-content">
              <div className="invest-modal-investor-bar">
                <div className="invest-modal-investor-bar__title">Company Payments Report</div>
              ...
              <div className="invest-modal-body">
                ...
                <div className="overall-cards">
                  <div className="header-gradient grad-paid">
                    ...
                    <div className="header-value">{formatINR(overallTotalsAllDepartments.netPaid ?? overallTotalsAllDepartments.paid)}</div>
                  ...
                  <div className="header-gradient grad-balance">
                    ...
                    <div className="header-value">{formatINR(overallTotalsCurrentDepartment.netPaid ?? overallTotalsCurrentDepartment.paid)}</div>
                  ...
                  <div className="row g-3 mb-4">
                    <div className="col-lg-8">
                      <div className="glass-card p-3">
                        ...
                        <BarChart data={monthlyChartData} width={520} height={200} color="url(#gradCompany)" />
                      </div>
                    </div>
                ...
                <div className="month-cards">
                  <div className="summary-card grad-paid">
                    ...
                    <div className="card-value">{renderAmount(monthlyTotals.net ?? monthlyTotals.paid)}</div>
                ...
                {activeYear && (
                  <div className="year-cards">
                    <div className="summary-card grad-paid">
                      ...
                      <div className="card-value">{renderAmount(yearlyTotals.net ?? yearlyTotals.paid)}</div>
                        ...
                        <table className="table table-sm table-hover invest-table">
                          ...
                          <tbody>
                            ...
                            {paginatedRows.map((r, i) => (
                              <tr key={`${r._companyDbKey}_${i}`} className="invest-table-row clickable-row" onClick={() => handleRowClick(r)}>
                                <td>{pageStart + i + 1}</td>
                                <td>{r.department || "-"}</td>
                                <td>
                                ...
                                <td>
                                  <div className="small">
                                    <div>JenCeo: {r.jenceoInvoiceNo || "-"}</div>
                                    <div>Company: {r.companyInvoiceNo || "-"}</div>
                                  </div>
                                </td>
                          ...
                          <tfoot>
                            <tr className="table-secondary">
                              ...
                              <td className="text-end">
                                <strong>{renderAmount(monthlyTotals.net ?? monthlyTotals.paid)}</strong>
                              </td>
                              <td className="text-end">
                            ...
                            <tr className="table-secondary">
                              ...
                              <td className="text-end">
                                <strong>{renderAmount(yearlyTotals.net ?? yearlyTotals.paid)}</strong>
                              </td>
                              <td className="text-end">
                      ...
                      <div className="table-responsive">
                        <table className="table table-sm table-hover invest-table">
                          ...
                          <tbody>
                            ...
                            {paginatedRows
                              .map((r, i) => (
                                <tr key={`${r._companyDbKey}_yr_${i}`} className="invest-table-row clickable-row" onClick={() => handleRowClick(r)}>
                                  <td>{pageStart + i + 1}</td>
                                  <td>{r.department || "-"}</td>
                                  <td>
                                    <div className="d-flex align-items-center gap-2">
              ...
              <div className="invest-modal-footer">
                <div className="me-auto small text-muted">
                  <strong>All Departments Total:</strong> {formatINR(overallTotalsAllDepartments.netPaid ?? overallTotalsAllDepartments.paid)} | 
                  <strong> {activeDepartment} Total:</strong> {formatINR(overallTotalsCurrentDepartment.netPaid ?? overallTotalsCurrentDepartment.paid)}
                </div>
              ...
              <div className="modal-footer">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    handleCompanyProfileClick(selectedPayment);
                  }}
                >
...
CompanyPaymentCard.propTypes = {
  companyCollections: PropTypes.shape({ active: PropTypes.string, exit: PropTypes.string }),
  openCompanyModal: PropTypes.func,
  rootPathForCompanyProfile: PropTypes.string,
};