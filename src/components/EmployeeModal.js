...
const EmployeeModal = ({ employee, isOpen, onClose, onSave, onDelete, isEditMode }) => {
  ...
  useEffect(() => {
    if (employee) {
      setFormData({
        ...employee,
        // ensure arrays exist with at least one blank section
        payments: Array.isArray(employee.payments) && employee.payments.length
          ? employee.payments
          : [blankPayment()],
        workDetails: Array.isArray(employee.workDetails) && employee.workDetails.length
          ? employee.workDetails
          : [blankWork()],
      });
      setStatus(employee.status || "On Duty");
      // reset errors when new employee loads
      setPaymentErrors(employee.payments?.map(() => ({})) || [{}]);
      setWorkErrors(employee.workDetails?.map(() => ({})) || [{}]);
    }
  }, [employee]);
  ...
  const handleSave = () => {
    if (!validate()) {
      // jump to first tab with errors if needed
  ...
  // small helper to render error under a field
              ...
              {activeTab === "basic" && (
                <div className="modal-card mb-3">
                  ...
                  <div className="modal-card-body">
                    <div className="row">
                      <div className="col-md-4">
                        {/* 1) Disable ID No in edit mode */}
                        {renderInputField("ID No", "idNo", formData.idNo || formData.employeeId, "text", "", true /* force disabled */)}
                      </div>
                  ...
                  <div className="modal-card-body">
                    {(formData.payments || []).map((p, i) => (
                      <div key={i} className="border rounded p-3 mb-3">
                        <h6 className="mb-3">Payment #{i + 1}</h6>
                        <div className="row">
