// HomeCareWorkerDetails.js
import React, { useState, useEffect } from "react";

const HomeCareWorkerDetails = ({ formData, handleChange, addWorker, removeWorker, errors = {}, setErrors, isViewMode = false }) => {
            ...
            <div className="col-md-6">
              <div className="form-group">
                <label htmlFor={`workerIdNo-${index}`}>
                  Worker ID No <span className="text-danger">*</span>
                </label>
                <input
                  type="text"
                  id={`workerIdNo-${index}`}
            ...
            <div className="col-md-6">
              <div className="form-group">
                ...
                <input
                  type="text"
                  id={`name-${index}`}
                  className={`form-control ${getErr(index, "name") ? "is-invalid" : ""}`}
                  name="name"
                  value={worker.name}
                  onChange={(e) => handleChange(e, "workers", index)}
                  readOnly={isViewMode}
                />
                {getErr(index, "name") && (
                  <div className="invalid-feedback">{getErr(index, "name")}</div>
                )}
              </div>
            </div>
          </div>
...
export default HomeCareWorkerDetails;