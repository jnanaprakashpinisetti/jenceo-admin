// ClientModal/utils.js - Housekeeping Version
...
export const friendlyLabel = (path) => {
  ...
  const topMap = {
    ...
    servicePeriod: "Service Frequency",
    serviceCharges: "Service Charges",
    preferredTime: "Preferred Time",
    serviceHours: "Service Hours/Day",
  };
...
export const buildClientBiodataHTML = (formData, headerImage) => {
  ...
  const serviceFields = [
    ["Service Frequency", safe(formData.servicePeriod)],
    ["Starting Date", safe(formData.startingDate)],
    ["Ending Date", safe(formData.endingDate)],
    ["Preferred Time", safe(formData.preferredTime)],
    ["Service Hours/Day", safe(formData.serviceHours)],
    ["Service Status", safe(formData.serviceStatus)],
    ["Special Instructions", safe(formData.serviceRemarks)],
  ];
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><title>Housekeeping Client Biodata - ${fullName}</title>
    <style>
  ...
  <div class="page">
    ...
    <div><h1>Housekeeping Client Biodata</h1><div class="muted" style="text-align:center">${metaDate}</div></div>
    ...
    <div class="section">
      <h3>Service Details</h3>
       <div className="table-responsive">
      <table><tbody>${renderPairs(serviceFields)}</tbody></table>
    </div>
    </div>
    ...
    <div class="biodata-footer" style="position:fixed;left:0;right:0;bottom:0;width:100%;background:#05b6ff;color:#fff;font-size:10px;padding:6px 8px;text-align:center;">
      Doc Ref: JC-HK-01 &nbsp;&nbsp;|&nbsp;&nbsp; Revision: 1 &nbsp;&nbsp;|&nbsp;&nbsp; Date: 1<sup>st</sup> May 2025 &nbsp;&nbsp;|&nbsp;&nbsp; Page 1 of 1
    </div>
