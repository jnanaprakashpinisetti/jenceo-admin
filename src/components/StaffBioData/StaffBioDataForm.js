...
// Full file — robust auto-ID generator that includes ExitStaffs node when scanning.
// It reads staff IDs from many likely DB nodes (including ExitStaffs), prefers JW prefix,
...
const WorkerBioDataForm = ({ isOpen = false, onClose = () => { }, onSaved }) => {
  ...
  // -------------------------------
  // Auto-generate ID (includes ExitStaffs node)
  // - Always runs when modal opens
  // - Scans nodes: ExitStaffs, StaffBioData, WorkerBioData, Staffs, EmployeeData
  ...
  useEffect(() => {
    ...
    async function fetchAndGenerateId() {
      try {
        ...
        // fallback: if none found, still attempt StaffBioData once more
        if (allIds.length === 0) {
          try {
        ...
        if (!mounted) return;
        setFormData((prev) => ({ ...prev, idNo: "JC01" }));
        setIdAutoGenerated(true);
        setTimeout(() => {
          const idEl = document.querySelector('input[name="idNo"]');
          if (idEl) idEl.setAttribute("disabled", "true");
  ...
  // Look up a staff record by idNo (case-insensitive convenience)
  const findStaffByIdNo = async (idNoRaw) => {
    ...
    // 2) uppercase fallback (helps if data saved uppercase)
  ...
  const handleBlur = async (e) => {
    ...
    if (name === "dateOfBirth") {
      if (value && !isOver18(value)) fieldErrors[name] = "Staff must be at least 18 years old";
      ...
      if (!raw) {
        ...
        try {
          ...
          if (hit && hit.rec) {
            ...
            // clear any earlier error on superiorId
            setErrors(prev => { const n = { ...prev }; delete n.superiorId; return n; });
          } else {
            // Not found — clear name and mark a gentle error
            setFormData(prev => ({ ...prev, superiorName: "" }));
            setErrors(prev => ({ ...prev, superiorId: "No staff found with this ID" }));
          }
        } catch (err) {
          // On error, do not block UX; just clear name and set a message
          setFormData(prev => ({ ...prev, superiorName: "" }));
        ...
        <div className="wb-card" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="workerFormTitle">
          <div className="wb-header">
            <div style={{ display: "flex", flexDirection: "column" }}>
              <div id="workerFormTitle" className="wb-title">Staff / Staff Form</div>
