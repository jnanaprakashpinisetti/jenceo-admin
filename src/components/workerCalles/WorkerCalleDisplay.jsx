...
const formatTime = (dateLike, mode = "12hr") => {
  const d = parseDate(dateLike); if (!isValidDate(d)) return "";
  const opts = mode === "24hr" ? { hour12: false, hour: "2-digit", minute: "2-digit" }
...
function collectWorkersFromSnapshot(rootSnap) {
  ...
  const walk = (snap, depth = 1) => {
    if (!snap) return;
    const val = snap.val();
    if (isWorkerShape(val)) { rows.push({ id: snap.key, ...val }); return; }
...
const normalizeSource = (raw) => {
  ...
  const map = new Map([
    ...
    ["reference", "Reference"], ["poster", "Poster"], ["agent", "Agent"],
    ["facebook", "Facebook"], ["linkedin", "LinkedIn"], ["linked in", "LinkedIn"],
    ["instagram", "Instagram"], ["youtube", "YouTube"], ["you tube", "YouTube"],
    ["website", "Website"], ["just dial", "Just Dial"], ["justdial", "Just Dial"], ["justdail", "Just Dial"],
...
const formatWorkerId = (firebaseId, index) => `WC-${String(index + 1).padStart(3, "0")}`;

/* =============================
   Permissions
   ============================= */
const derivePermissions = (currentUserRole, explicit) => {
  const base = { canView: true, canEdit: false, canDelete: false, canExport: false };
  ...
  if (role === "admin") return { canView: true, canEdit: true, canDelete: true, canExport: true };
  if (role === "editor" || role === "manager") return { canView: true, canEdit: true, canDelete: false, canExport: false };
  return base; // viewer
};
...
/* =============================
   Component
   ============================= */
export default function WorkerCalleDisplay({ currentUserRole = "admin", permissions: permissionsProp }) {
  // permissions
  ...
  // options
  const callThroughOptions = ["Apana", "WorkerIndian", "Reference", "Poster", "Agent", "Facebook", "LinkedIn", "Instagram", "YouTube", "Website", "Just Dial", "News Paper", "Other"];
  ...
  }, [workers, searchTerm, selectedGender, selectedSkills, selectedRoles, selectedLanguages, skillMode, reminderFilter, selectedSource, ageRange]);
  ...
  useEffect(() => { setCurrentPage(1); }, [searchTerm, selectedGender, selectedSkills, selectedRoles, selectedLanguages, skillMode, reminderFilter, selectedSource, ageRange, rowsPerPage]);
  ...
  const handleView = (w) => {
    ...
    setSelectedWorker(w); setIsEditMode(false); setIsModalOpen(true);
  };
  ...
  const handleDelete = (w, e) => {
    ...
    setSelectedWorker(w); setShowDeleteConfirm(true);
  };
  ...
  const handleDeleteConfirmed = () => {
    ...
    setShowDeleteConfirm(false); setDeleteReason(""); setShowDeleteReason(true);
  };
  const performDeleteWithReason = async () => {
    ...
    try {
      setIsDeleting(true);
      const payload = { ...selectedWorker, originalId: selectedWorker.id, deletedAt: new Date().toISOString(), deleteReason: deleteReason.trim() };
    ...
    } catch (err) { console.error(err); alert("Error deleting worker"); } finally { setIsDeleting(false); }
  };
  ...
  const resetFilters = () => {
    ...
    setReminderFilter(""); setSelectedSource("All"); setSkillMode("single"); setTimeFormat("12hr"); setAgeRange({ min: "", max: "" });
  ...
  return (
    <div className="workerCalls">
      ...
      <div className="d-flex justify-content-between flex-wrap gap-2 mb-3 p-2 bg-dark border rounded-3">
        ...
        <input type="text" className="form-control opacity-75" placeholder="Search name, location, mobile…" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={{ maxWidth: 325 }} />
      ...
      <div className="p-3 mb-3 bg-dark border rounded-3">
        <div className="row g-3 align-items-center">
          <div className="col-md-3 text-center">
            ...
            <div className="d-flex gap-2 justify-content-center">
              {["Male", "Female", "Other"].map(g => {
                ...
                return (
                  <button key={g} type="button" className={`btn ${on ? "btn-warning" : "btn-outline-warning"} btn-sm`}
                    onClick={() => setSelectedGender(prev => on ? prev.filter(x => x !== g) : [...prev, g])}>{g}</button>
                );
              })}
            </div>
          </div>
          <div className="col-md-3 text-center">
            ...
            <div className="d-flex gap-2 justify-content-center">
              <button type="button" className={`btn ${skillMode === "single" ? "btn-info" : "btn-outline-info"} btn-sm`} onClick={() => setSkillMode("single")}>Any</button>
              <button type="button" className={`btn ${skillMode === "multi" ? "btn-info" : "btn-outline-info"} btn-sm`} onClick={() => setSkillMode("multi")}>All</button>
            </div>
          </div>
          <div className="col-md-3 text-center">
            <label className="form-label text-white small mb-2">Time</label>
            <div className="d-flex gap-2 justify-content-center">
              <button type="button" className={`btn ${timeFormat === "12hr" ? "btn-primary" : "btn-outline-primary"} btn-sm`} onClick={() => setTimeFormat("12hr")}>12hr</button>
              <button type="button" className={`btn ${timeFormat === "24hr" ? "btn-primary" : "btn-outline-primary"} btn-sm`} onClick={() => setTimeFormat("24hr")}>24hr</button>
            </div>
          </div>
          <div className="col-md-3 text-center">
            <label className="form-label text-white small mb-2">Job Roles</label>
      ...
      <div className="row g-3 mb-3">
          ...
          <div className="p-3 bg-dark border rounded-3 h-100">
            ...
            <div className="d-flex flex-wrap gap-2">
              {languageOptions.map(l => {
                ...
                return (
                  <button key={l} className={`btn btn-sm ${active ? "btn-info text-dark" : "btn-outline-info"} rounded-pill`}
                    onClick={() => setSelectedLanguages(prev => active ? prev.filter(x => x !== l) : [...prev, l])}>{l}</button>
                );
              })}
            </div>
          </div>
        ...
        <div className="col-md-6">
          <div className="p-3 bg-dark border rounded-3 h-100">
            <h6 className="mb-2 text-info">Housekeeping Skills</h6>
            <div className="d-flex flex-wrap gap-2">
              {skillOptions.map(s => {
                const active = selectedSkills.includes(s);
                return (
                  <button key={s} className={`btn btn-sm ${active ? "btn-outline-light border-info" : "btn-outline-secondary"} rounded-pill`}
                    onClick={() => setSelectedSkills(prev => active ? prev.filter(x => x !== s) : [...prev, s])}>{s}</button>
                );
              })}
            </div>
          </div>
        ...
        <div className="p-3 bg-dark border rounded-3 mb-3">
          ...
          <div className="d-flex flex-wrap gap-2">
            {roleOptions.map(r => {
              ...
              return (
                <button key={r} className={`btn btn-sm ${active ? "btn-success" : "btn-outline-success"} rounded-pill`}
                  onClick={() => setSelectedRoles(prev => active ? prev.filter(x => x !== r) : [...prev, r])}>{r}</button>
              );
            })}
          </div>
        </div>
                ...
                <tr key={w.id} onClick={(e) => handleRowClick(w, e)} style={{ cursor: "pointer" }} className={urgencyClass(w?.callReminderDate)}>
                  ...
                  <td className="text-white">
                    ...
                    {w?.mobileNo && (
                      <div className="mt-1" onClick={(e) => e.stopPropagation()}>
                        ...
                        <a className="btn btn-sm btn-outline-success rounded-pill"
                          href={`https://wa.me/${String(w.mobileNo).replace(/\D/g, "")}?text=${encodeURIComponent("Hello, This is Sudheer From JenCeo Home Care Services")}`}
                          target="_blank" rel="noopener noreferrer">WAP</a>
                      </div>
                    )}
                  </td>
                  <td>
                  ...
                  <td className="text-nowrap" onClick={(e) => e.stopPropagation()}>
                    <div className="btn-group" role="group">
                      {permissions.canView && (
                        <button className="btn btn-sm btn-outline-info rounded-pill" title="View" onClick={() => handleView(w)}>
                          <img src={viewIcon} alt="view" width="16" height="16" />
                        </button>
                      )}
                      {permissions.canEdit && (
                        <button className="btn btn-sm btn-outline-light border-warning rounded-pill" title="Edit" onClick={(e) => handleEdit(w, e)}>
                          <img src={editIcon} alt="edit" width="16" height="16" />
                        </button>
                      )}
                      {permissions.canDelete && (
                        <button className="btn btn-sm btn-outline-danger rounded-pill" title="Delete" onClick={(e) => handleDelete(w, e)}>
        ...
        <div className="modal show d-block" tabIndex="-1" style={{ backgroundColor: "rgba(0,0,0,0.8)" }}>
          <div className="modal-dialog"><div className="modal-content">
            <div className="modal-header">
              ...
              <button type="button" className="btn-close" onClick={() => { setShowDeleteReason(false); setDeleteReason(""); }} disabled={isDeleting} />
            </div>
            <div className="modal-body">
              <p>Please provide a reason for deleting <strong>{selectedWorker?.name || "this worker"}</strong>:</p>
              <textarea className="form-control" rows="3" value={deleteReason} onChange={(e) => setDeleteReason(e.target.value)} placeholder="Enter reason…" disabled={isDeleting} />
            </div>
            <div className="modal-footer">
              <button type="button" className="btn btn-secondary" onClick={() => { setShowDeleteReason(false); setDeleteReason(""); }} disabled={isDeleting}>Cancel</button>
              <button type="button" className="btn btn-danger" onClick={performDeleteWithReason} disabled={isDeleting || !deleteReason.trim()}>
